...
class TestForecastGenerator(unittest.TestCase):
    ...
    def test_make_forecasts(self):
        forecasts = fg.make_forecasts(TEST_VALUES)
        self.assertEqual(len(forecasts), 24*len(TEST_VALUES))
        self.assertTrue(type(forecasts[0] is np.ndarray))



    def test_make_24h_forecasts(self):
        values_ = TEST_VALUES[0:25]
        self.assertEqual(len(values_), 25)
        self.assertEqual(len(values_[0]), 6)


        forecasts = fg.make_24h_forecasts(values_)

        # expect error on less than 25 values
        self.assertRaises(IndexError, fg.make_24h_forecasts, values_[0:10])
        ...
        self.assertEqual(len(forecasts[0]), 7)



    def test_merge_forecast_data(self):
        forecasts = TEST_VALUES[1, 2:]
        origin = TEST_VALUES[0, 0]
        date = TEST_VALUES[1, 0]
        forecast_item = fg.merge_forecast_data(forecasts, date, origin)
        self.assertEqual(len(forecast_item), 7)
...
TEST_VALUES = np.array([[datetime.datetime(2009, 1, 1, 0, 0), 'rotterdam', -1.5, 1.0, 240, 1.0],
